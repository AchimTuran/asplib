...
namespace asplib
{
  struct DecimatorOptions : ASPLIB_EXTENDED_STRUCT(DecimatorOptions)
  {
    uint32_t decimationFactor;
    uint32_t filterLength;
    float cutoffFrequency;

    DecimatorOptions() : ASPLIB_EXTENDED_STRUCT_REGISTER(ASPLIB_EXTENDED_STRUCT_DecimatorOptions)
    {
      decimationFactor = 2;
      filterLength = 101;
